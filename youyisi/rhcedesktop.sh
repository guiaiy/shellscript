...
nmcli connection modify team0 ipv4.method manual ipv4.addresses 172.16.3.25/24 connection.autoconnect yes &>/dev/null
...
#echo 'local_transport = error:haha' >> /etc/postfix/main.cf
systemctl enable postfix &>/dev/null
systemctl restart postfix
sleep 1
echo 'postfix success'
echo 'samba installing please wait'
output=`yum -y install cifs-utils.x86_64` &>/dev/null
mkdir /mnt/dev
...
#
#
#creating script please wait
#
#'
#echo '#!/bin/bash
ecking MariaDB deployment .. .. pass
22. checking MariaDB select .. .. skiped
##if [ "$1" == "redhat" ];then
#echo "fedora"
#elif [ "$1" == "fedora" ];then
#echo "redhat"
...
output=`yum -y install iscsi-initiator-utils.1686` &> /dev/null
...
iscsiadm --mode discoverydb --type sendtargets --portal 172.25.0.11 --discover &>/dev/null
output=`systemctl restart iscsi`
echo 'n
...
w' > /root/iscsi.txt
sleep 1
fdisk /dev/sda < /root/iscsi.txt &> /dev/null
output=`partprobe`
output=`mkfs.ext4 /dev/sda1` &>/dev/null
mkdir /mnt/data
...
#echo '
#
#mariadb installing please wait
#
#'
...
mount -a
sleep 5
done &' > /etc/rc.d/mount.sh
chmod +x /etc/rc.d/mount.sh
echo 'sh /etc/rc.d/mount.sh' >> /etc/rc.d/rc.local
echo 'mount success'
